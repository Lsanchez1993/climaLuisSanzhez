...
.controller('DashCtrl', function($scope, $http, Chats, Units, Q, C) {
  ...
  //
    $http.get("http://api.openweathermap.org/data/2.5/forecast/daily?q="+Q.get()+"&cnt=7&units=" + Units.get()).success(function (data){
      Chats.set(data.list);
...
.controller('HrsCtrl', function($scope, $http, Chats, Units, Q, C) {

  //
    $http.get("http://api.openweathermap.org/data/2.5/forecast?q="+Q.get()+"&cnt=7&units=" + Units.get()).success(function (data){
      Chats.set(data.list);
      console.log(data.list);
      $scope.weatherToday = Chats.getOne();
      $scope.chats = Chats.all();
